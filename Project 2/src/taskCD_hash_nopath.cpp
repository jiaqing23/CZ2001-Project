...
int hospital[MAX_N];
vector<int> G[MAX_N];
vector<pi> hospt_dist[MAX_N]; //(hospital, distance)
unordered_set<int> hospt[MAX_N];
...
void bfs(){
	queue<bfsElm> que;
	for(int i = 0; i < h; i++){
		que.push({hospital[i], hospital[i], 0});
	...
	while(!que.empty()){
		...
		for(int i = 0; i < (int)G[now.v].size(); i++){
			...
			if((int)hospt_dist[to].size() < k){
				...
				que.push({to, now.hospt, now.dist+1});
...
int main(int argc, char** argv){
	...
	for(int i = 0; i < n; i++){
		...
		for(int j = 0; j < k; j++){
			...
			if((int)hospt_dist[i].size() < j + 1) {
				fprintf(f, ": Not exists\n");
				continue;
			}
		
			fprintf(f, ": %d (distance = %d)\n", hospt_dist[i][j].fi, hospt_dist[i][j].se);
		}
	}
	fprintf(f, "--------------------------------------------------------------\n");
	fclose(f);
